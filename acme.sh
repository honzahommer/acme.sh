...
installcert() {
  ...
  if [ -z "$_main_domain" ]; then
    _usage "Usage: $PROJECT_ENTRY --install-cert --domain <domain.tld> [--ecc] [--cert-file <file>] [--key-file <file>] [--ca-file <file>] [ --reloadcmd <command>] [--fullchain-file <file>] [--fullchain-key-file <file>]"
    return 1
  fi
...
Parameters:
  ...
  --fullchain-key-file <file>       After issue/renew, the fullchain cert with key will be copied to this path.
